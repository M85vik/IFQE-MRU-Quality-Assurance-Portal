...
const Indicator343Preview = ({ fileKey }) => {
  ...
  useEffect(() => {
    ...
    const fetchAndParseExcel = async () => {
      ...
      try {
        ...
        // 3.4.3 Template columns:
        // S. No. | Title of the Awareness Session | SDG Mapped | Name of Resource Person
        ...
        for (let i = 0; i < Math.min(rows.length, 5); i++) {
          ...
          row.forEach((cell, idx) => {
            ...
            if (text.includes('title of the awareness') || text.includes('awareness session') || (text.includes('title') && !text.includes('resource'))) {
              colTitle = idx;
      ...
      <div className="mt-6 w-full rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden animate-in fade-in duration-300">
        ...
        <div className="p-4 border-b border-gray-100 bg-gray-50/80 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Automated validation of uploaded awareness sessions.</p>
          </div>
          <div className="flex-shrink-0 w-11 h-11 rounded-full bg-rose-100 flex items-center justify-center border-2 border-rose-200">
            <span className="text-lg font-bold text-rose-700">{summaryData.totalCount}</span>
          </div>
        </div>
              ...
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <td
                    ...
                    style={{ fontSize: '11px' }}
                  >
                    3.4.3 Awareness session organized on life skills
                  </td>
                </tr>
              ...
              <tbody>
                {summaryData.programs.map((prog, idx) => (
                  <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-3 py-2 text-center font-bold text-rose-700 bg-rose-50/30" style={{ fontSize: '11px' }}>
                      {prog.count}
                    </td>
                  </tr>
                ))}
                ...
                {summaryData.programs.length > 0 && (
                  <tr className="bg-gray-50/80 border-t-2 border-gray-200">
                    ...
                    <td className="px-3 py-2 text-center font-bold text-rose-800" style={{ fontSize: '11px' }}>
...
export default Indicator343Preview;
