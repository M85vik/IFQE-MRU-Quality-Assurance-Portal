...
const Indicator344Preview = ({ fileKey }) => {
  ...
  useEffect(() => {
    ...
    const fetchAndParseExcel = async () => {
      ...
      try {
        ...
        // 3.4.4 Template columns:
        // s.no | Title of Awareness Session | SDG Mapped | Name of Resource Person
        ...
        for (let i = 0; i < Math.min(rows.length, 5); i++) {
          ...
          row.forEach((cell, idx) => {
            ...
            if ((text.includes('awareness') && text.includes('title')) || (text.includes('title') && !text.includes('resource'))) {
              colTitle = idx;
      ...
      <div className="mt-6 w-full rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden animate-in fade-in duration-300">
        <div className="p-4 border-b border-gray-100 bg-gray-50/80 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Automated validation of soft skills awareness sessions.</p>
          </div>
          <div className="flex-shrink-0 w-11 h-11 rounded-full bg-violet-100 flex items-center justify-center border-2 border-violet-200">
            <span className="text-lg font-bold text-violet-700">{summaryData.totalCount}</span>
          </div>
        </div>
              ...
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <td colSpan="2" className="px-3 py-2 font-bold text-gray-800 text-left" style={{ fontSize: '11px' }}>
                    3.4.4 Awareness session organized on Soft Skills, Language and communications Skills
                  </td>
                </tr>
              ...
              <tbody>
                {summaryData.programs.map((prog, idx) => (
                  <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-3 py-2 text-center font-bold text-violet-700 bg-violet-50/30" style={{ fontSize: '11px' }}>{prog.count}</td>
                  </tr>
                ))}
                {summaryData.programs.length === 0 && (
                  <tr>
                    <td colSpan="2" className="px-3 py-3 text-center text-gray-400 italic" style={{ fontSize: '11px' }}>
                      No awareness session data found in the uploaded file.
                    </td>
                  </tr>
                )}
                {summaryData.programs.length > 0 && (
                  <tr className="bg-gray-50/80 border-t-2 border-gray-200">
                    ...
                    <td className="px-3 py-2 text-center font-bold text-violet-800" style={{ fontSize: '11px' }}>{summaryData.totalCount}</td>
                  </tr>
                )}
              </tbody>
            </table>
...
export default Indicator344Preview;
