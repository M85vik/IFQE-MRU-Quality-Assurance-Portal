  ...
  useEffect(() => {
    ...
    const fetchAndParseExcel = async () => {
      ...
      try {
        ...
        // Row 0: Title header "1.1.5 (a) Percentage of Courses having focus on employability..."
        // Row 1: Column headers: Program Name | Total Courses for Batch | No. of Courses having focus... | Percentage of courses
        // Row 2+: Data rows
        // We skip header rows (first 2-3 rows) and parse data

        let programs = [];
        let totalCoursesSum = 0;
        let focusCoursesSum = 0;
        ...
        for (let i = dataStartRow; i < rows.length; i++) {
          ...
          const focusCourses = parseNumber(row[2]);
          const percentage = row[3] !== undefined && row[3] !== null && row[3] !== ''
            ? parseNumber(row[3])
            : (totalCourses > 0 ? ((focusCourses / totalCourses) * 100) : 0);

          if (programName && (totalCourses > 0 || focusCourses > 0)) {
            programs.push({
              programName,
              totalCourses,
              focusCourses,
            ...
            focusCoursesSum += focusCourses;
          }
        }

        const overallPercentage = totalCoursesSum > 0
          ? Math.round((focusCoursesSum / totalCoursesSum) * 10000) / 100
          : 0;
  ...
  const findDataStartRow = (rows) => {
    for (let i = 0; i < Math.min(rows.length, 6); i++) {
      ...
      if (firstCell.includes('program') || firstCell.includes('percentage') || 
          firstCell.includes('1.1.5') || firstCell.includes('course') ||
                ...
                <table className="w-full text-xs text-left">
                  <thead className="text-[11px] font-bold text-gray-500 bg-gray-50">
                    <tr>
                      ...
                      <th className="px-3.5 py-2 font-bold text-center">FOCUS COURSES</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {summaryData.programs.map((item, idx) => (
                      <tr key={idx} className="bg-white hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-3.5 py-2.5 text-gray-600 text-center">{item.focusCourses}</td>
                        <td className="px-3.5 py-2.5 text-right">
                          <span className={`inline-flex px-2 py-0.5 rounded text-[11px] font-bold tracking-wide ${
                            item.percentage >= 75 ? 'bg-emerald-100 text-emerald-700' :
                            item.percentage >= 60 ? 'bg-blue-100 text-blue-700' :
                            item.percentage >= 40 ? 'bg-yellow-100 text-yellow-700' :
                            item.percentage >= 20 ? 'bg-orange-100 text-orange-700' :
                  ...
                  <tfoot className="bg-gray-50 border-t-2 border-gray-200">
                    <tr>
                      ...
                      <td className="px-3.5 py-2.5 font-bold text-gray-800 text-center">{summaryData.focusCoursesSum}</td>
                      <td className="px-3.5 py-2.5 font-bold text-right">
                        <span className={`inline-flex px-2.5 py-1 rounded text-[12px] font-bold tracking-wide ${
                          summaryData.overallPercentage >= 75 ? 'bg-emerald-100 text-emerald-700' :
                          summaryData.overallPercentage >= 60 ? 'bg-blue-100 text-blue-700' :
                          summaryData.overallPercentage >= 40 ? 'bg-yellow-100 text-yellow-700' :
                          summaryData.overallPercentage >= 20 ? 'bg-orange-100 text-orange-700' :
                          'bg-red-100 text-red-700'
                        }`}>
