...
const Indicator216Preview = ({ fileKey }) => {
  ...
  useEffect(() => {
    ...
    const fetchAndParseExcel = async () => {
      ...
      try {
        ...
        // Column indices for 2.1.6 template:
        // S.No | Title of Event | SDG Mapped | Name of Resource Person
        ...
        for (let i = 0; i < Math.min(rows.length, 5); i++) {
          ...
          row.forEach((cell, idx) => {
            ...
            if (text.includes('title of event') || (text.includes('title') && !text.includes('resource'))) {
              colTitle = idx;
        ...
        for (let i = dataStart; i < rows.length; i++) {
          ...
          const participants = row[colParticipants];
          if (participants !== undefined && participants !== null && participants !== '') {
            const num = parseInt(String(participants).replace(/[^0-9]/g, ''), 10);
            if (!isNaN(num)) totalParticipants += num;
          }
        }

        setSummaryData({ totalEvents, totalParticipants });
      } catch (err) {
      ...
      <div className="mt-6 w-full rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden animate-in fade-in duration-300">
        ...
        <div className="p-4 border-b border-gray-100 bg-gray-50/80 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Automated validation of uploaded seminars / workshops / training programs.</p>
          </div>
          <div className="flex-shrink-0 w-11 h-11 rounded-full bg-teal-100 flex items-center justify-center border-2 border-teal-200">
            <span className="text-lg font-bold text-teal-700">{summaryData.totalEvents}</span>
          </div>
        </div>
              ...
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <td
                    ...
                    style={{ fontSize: '11px' }}
                  >
                    2.1.6 Research - Seminars/ Workshop/Training programs organized
                  </td>
                </tr>
                <tr className="border-b border-gray-200 bg-gray-100/60 text-gray-600 font-bold" style={{ fontSize: '11px' }}>
                  <th className="px-3 py-2 text-left border-r border-gray-200 w-[60%]"></th>
                  <th className="px-3 py-2 text-center w-[40%]">Total count</th>
                </tr>
              </thead>
              <tbody>
                <tr className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-3 py-2.5 font-semibold text-gray-700 border-r border-gray-200" style={{ fontSize: '11px' }}>
                    Seminars/ Workshop/Training programs organized
                  </td>
                  <td className="px-3 py-2.5 text-center font-bold text-teal-700 bg-teal-50/30" style={{ fontSize: '11px' }}>
...
export default Indicator216Preview;
