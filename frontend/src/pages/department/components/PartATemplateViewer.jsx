...
const PartATemplateViewer = () => {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h3 className="text-lg font-semibold text-white mb-2">Template Contents</h3>
                ...
                <div className="space-y-2 border rounded-lg p-4 bg-gray-80">
                    {templateSections.map(section => (
                        <div key={section.code} className="flex items-start">
                            <FileText className="h-4 w-4 text-white mt-1 mr-3 flex-shrink-0" />
                            <p className="text-white"><strong>{section.code}. {section.title}</strong></p>
                        </div>
                    ))}
                </div>
            </div>
