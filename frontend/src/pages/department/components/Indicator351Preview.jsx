...
const Indicator351Preview = ({ fileKey }) => {
  ...
  useEffect(() => {
    ...
    const fetchAndParseExcel = async () => {
      ...
      try {
        ...
        // 3.5.1 Template columns:
        // S.No | Name of Club/Student Chapter | Faculty Coordinator | Student Coordinator
        // | Year of Establishment | Focus Area | Number of Active Members | Evidence Link
        let colClubName = 1;
        let colFocusArea = 5;
        ...
        for (let i = 0; i < Math.min(rows.length, 5); i++) {
          ...
          row.forEach((cell, idx) => {
            ...
            if (text.includes('name of the club') || text.includes('name of club') || text.includes('student chapter') || text.includes('club/student')) {
              colClubName = idx;
            } else if (text.includes('focus area')) {
              colFocusArea = idx;
            }
          });
        }
        ...
        // Group by Focus Area as "Program"
        const programMap = {};
        let totalCount = 0;
        ...
        for (let i = dataStart; i < rows.length; i++) {
          ...
          const clubName = row[colClubName] && String(row[colClubName]).trim();
          if (!clubName) continue;
          if (clubName.toLowerCase().includes('total')) continue;

          totalCount++;

          const focusArea = row[colFocusArea] && String(row[colFocusArea]).trim();
          const key = focusArea && focusArea !== '' ? focusArea : 'General';
      ...
      <div className="mt-6 w-full rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden animate-in fade-in duration-300">
        <div className="p-4 border-b border-gray-100 bg-gray-50/80 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Automated validation of student clubs / local chapters.</p>
          </div>
          <div className="flex-shrink-0 w-11 h-11 rounded-full bg-sky-100 flex items-center justify-center border-2 border-sky-200">
            <span className="text-lg font-bold text-sky-700">{summaryData.totalCount}</span>
          </div>
        </div>
              ...
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <td colSpan="2" className="px-3 py-2 font-bold text-gray-800 text-left" style={{ fontSize: '11px' }}>
                    3.5.1 Availability of Student clubs/local chapters in the schools
                  </td>
                </tr>
              ...
              <tbody>
                {summaryData.programs.map((prog, idx) => (
                  <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-3 py-2 text-center font-bold text-sky-700 bg-sky-50/30" style={{ fontSize: '11px' }}>{prog.count}</td>
                  </tr>
                ))}
                {summaryData.programs.length === 0 && (
                  <tr>
                    <td colSpan="2" className="px-3 py-3 text-center text-gray-400 italic" style={{ fontSize: '11px' }}>
                      No club data found in the uploaded file.
                    </td>
                  </tr>
                )}
                {summaryData.programs.length > 0 && (
                  <tr className="bg-gray-50/80 border-t-2 border-gray-200">
                    ...
                    <td className="px-3 py-2 text-center font-bold text-sky-800" style={{ fontSize: '11px' }}>{summaryData.totalCount}</td>
                  </tr>
                )}
              </tbody>
            </table>
...
export default Indicator351Preview;
